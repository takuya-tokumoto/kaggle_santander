...
def train_predict(all_df, features, prod_features, str_date, cv):
    ...
    test_add_list_lightgbm = make_submission(io.BytesIO() if cv else gzip.open("data/output/%s.lightgbm.csv.gz" % str_date, "wb"), Y_test_lgbm - Y_prev, C)
    ...
    test_add_list_xgboost = make_submission(io.BytesIO() if cv else gzip.open("data/output/%s.xgboost.csv.gz" % str_date, "wb"), Y_test_xgb - Y_prev, C)
    ...
    test_add_list_xl = make_submission(io.BytesIO() if cv else gzip.open("data/output/%s.xgboost-lightgbm.csv.gz" % str_date, "wb"), Y_test - Y_prev, C)
