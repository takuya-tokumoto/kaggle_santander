...
def xgboost(XY_train, XY_validate, test_df, features, XY_all=None, restore=False):
    ...
    if not restore:
        ...
        W_train = XY_train["weight"].values
        # xgboost 専用データ形式に変換します。
        train = xgb.DMatrix(X_train, label=Y_train, feature_names=features, weight=W_train)

        # 検証データに対して同じ作業を進めます。
        X_validate = XY_validate[features].values
        Y_validate = XY_validate["y"].values
        W_validate = XY_validate["weight"].values
    ...
    if XY_all is not None:
        ...
        X_all = XY_all[features].values
        Y_all = XY_all["y"].values
        W_all = XY_all["weight"].values
...
def lightgbm(XY_train, XY_validate, test_df, features, XY_all=None, restore=False):
    ...
    if not restore:
        ...
        model.save_model("lgbm.model.txt")
        pickle.dump(best_iteration, open("lgbm.model.meta", "wb"))
    else:
        # model = lgbm.Booster(model_file="tmp/lgbm.model.txt")
        # best_iteration = pickle.load(open("tmp/lgbm.model.meta", "rb"))
        model = lgbm.Booster(model_file="lgbm.model.txt")
        best_iteration = pickle.load(open("lgbm.model.meta", "rb"))
